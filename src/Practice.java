...
public class Practice {
  ...
  public static int oddVertices(Vertex<Integer> starting) {
    ...
    int count = oddVertices(starting, visited);
    return count;
  }
  ...
  public static List<Integer> sortedReachable(Map<Integer, Set<Integer>> graph, int starting) {
    List<Integer> sortedList = new LinkedList<>();
    if(!graph.containsKey(starting)) return sortedList;

    sortedList = graphReachable(graph, starting, new HashSet<>());
    Collections.sort(sortedList);

    return sortedList;
  }

  private static List<Integer> graphReachable(Map<Integer, Set<Integer>> graph, int starting, Set<Integer> visited) {
    if(visited.contains(starting)) return new LinkedList<>();
    ...
    for(Integer neighbor : graph.get(starting)) {
      reachable.addAll(graphReachable(graph, neighbor, visited));
    }
  ...
  private static <T> boolean canReach(Vertex<T> current, Vertex<T> end, Set<Vertex<T>> visited) {
    ...
    for(Vertex<T> neighbor : current.neighbors) {
      if(canReach(neighbor, end, visited)) {
        return true;
      }
    }
  ...
  private static boolean positivePathExists(Map<Integer, Set<Integer>> graph, int current, int ending, Set<Integer> visited) {
    ...
    for(Integer neighbor : graph.get(current)) {
      if(neighbor > 0 && positivePathExists(graph, neighbor, ending, visited)) {
        return true;
      }
    }
  ...
  public static List<int[]> nextMoves(char[][] board, int[] current, int[][] directions) {
    ...
    for(int[] d : directions) {
      int newR = current[0] + d[0];
      int newC = current[1] + d[1];
